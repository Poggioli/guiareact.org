...
const scaleToHSLObject = (name, scale) => {
...
const scaleToHexObject = (name, scale) => {
    const values = Object.entries(scale)
        .map(([key, value]) => {
...
const scaleToSASS = (scale) => {
    return Object.entries(scale)
        .map(([key, val]) => `$${key}: ${val};`)
        .join('\n');
};

const scaleToLESS = (scale) => {
    return Object.entries(scale)
        .map(([key, val]) => `@${key}: ${val};`)
        .join('\n');
};
...
const scaleToSvg = (
    ...
    return `<svg fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${values.length * valueWidth
        } ${valueHeight}">${values
            .map((value, i) => {
...
export const ColorScaleGroup = ({ children }) => {
    return (
        <>
            <Flex css={{ width: '100%' }}>
...
export const ColorScale = ({ label, name }) => {
